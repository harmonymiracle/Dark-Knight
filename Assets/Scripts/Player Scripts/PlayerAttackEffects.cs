...
public class PlayerAttackEffects : MonoBehaviour {
	...
	public GameObject groundImpactSpawn, kickFXSpawn, fireTornadoSpawn, fireShieldSpawn, healFXSpawn, thunderFXSpawn;
	public GameObject groundImpactPrefab, kickFXPrefab, fireTornadoPrefab, fireShieldPrefab, healFXPrefab, thunderFXPrefab;
	...
	// the actual anim effect can use anim event to express
	#region  Animation Events Function
	// this match the animation event name
	void GroundImpact () {
	...
	void FireShield () {
		GameObject fireObj = Instantiate (fireShieldPrefab, fireShieldSpawn.transform.position, Quaternion.identity);
	...
	public void ThunderAttack () {
		for (int i = 0; i < 8; i++) {
			...
			GameObject healObj = Instantiate (thunderFXPrefab, pos, Quaternion.identity) as GameObject;
